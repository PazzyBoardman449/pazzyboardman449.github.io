%% Solution to the Basic Morphogenisis equation for different flux magnitues but flux at both ends in fixed time scenario.
...
J2 = 0.33; %Flux at the boundary (µm^-2 s^-1)
J3 = 0.27; %Flux at the boundary (µm^-2 s^-1)
...
for k = 2:num_steps
    ...
    p2(k,num_latt_pts) = p2(k,num_latt_pts-1)+J2*Dx/D;
...
for m = 2:num_steps
    ...
    p3(m,num_latt_pts) = p3(m,num_latt_pts-1)+J3*Dx/D;
...
legend('ρ(x,t=100s), J = 0.3','ρ(x,t=100s), J = 0.33','ρ(x,t=100s), J = 0.27','Threshold ρ_{max}','fontsize',12)